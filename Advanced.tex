...
After you become familiar with printing using the default settings, a few advanced techniques may help in getting better and more consistent prints from the LulzBot\textsuperscript{\miniscule{\textregistered}} Mini 3D printer. Some of these instructions are items and materials not included with the Mini. With any of these additional items or materials, follow safety and usage guidelines as instructed by the manufacturer.
...
\label{ssec:num2}
We include the latest version of 3D printer firmware with each update of Cura LulzBot Edition. Download the latest version at \texttt{LulzBot.com/Cura} for the latest firmware and tested slicing profiles. Once downloaded and installed, select the correct 3D printer or use the \texttt{Machine > Add New Machine} process to add your 3D printer. To update your 3D printer firmware through Cura LulzBot Edition, select your 3D printer, power on and plug your 3D printer into the computer. Select \texttt{Machine > Install Default Firmware.} The firmware flashing process will then complete.
